...
description: Use bunqueue MCP server with Claude Desktop, Claude Code, Cursor, and any MCP client. 70 tools for complete queue management via natural language.
head:
  - tag: meta
    attrs:
      property: og:image
...
bunqueue includes an **MCP (Model Context Protocol)** server that gives AI assistants **complete control** over your job queues. 70 tools, 5 resources, zero configuration. Built on the official `@modelcontextprotocol/sdk` for full protocol compliance.
...
All 70 tools return structured error responses. When a tool encounters an error (backend exception, connection failure, invalid operation), it returns:
